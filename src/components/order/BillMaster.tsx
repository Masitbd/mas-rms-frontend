...
const BillMaster = (props: { mode: string }) => {
  const [
    getPrintingMode,
    { isLoading: printingModeLoading, isFetching: PrintingModeFetching },
  ] = useLazyGetPrintingModeQuery();
  ...
  const handlePrint = async (id: string) => {
    try {
      ...
      const mode = await getPrintingMode(undefined).unwrap();
      const doc = InvoiceDocProvider(
        mode?.data[0]?.cashMemoType as string,
        bill,
        session
      );

      // Printing on different window

      pdfMake
        .createPdf(doc as unknown as TDocumentDefinitions)
        .getBlob((result) => {
